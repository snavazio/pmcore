#!/usr/bin/env bash
# ============================================================
# PMCore v4 Retrain — Clean corpus + structured data
# ============================================================
set -euo pipefail
source ~/.pmcore_env

PYTHON="/home/snavazio/.local/bin/uv run python"
LOG_DIR="./retrain_logs_v4"
mkdir -p "$LOG_DIR"
...
log "Step 3: Rebuilding v4 corpus..."
$PYTHON rebuild_corpus_v4.py 2>&1 | tee "$LOG_DIR/rebuild.log"

# Verify corpus
V3_PATH="corpus/master_corpus_v4.jsonl"
if [ ! -f "$V3_PATH" ]; then
...
log "v4 corpus: $LINES records | $SIZE"

# Update train.py to point at v4 corpus
sed -i 's|corpus/master_corpus\.jsonl|corpus/master_corpus_v4.jsonl|g' pmcore/train.py
log "Updated train.py → corpus/master_corpus_v4.jsonl"
